...
你手里有这么多需求，需要可插拔式地把这些东西集成起来，供你做灵活调用。

这个时候，Semantic Kernel 的价值就来了。
...
* **插件**是可以被你的AI服务调用的功能。比如，你用插件实现数据库调用或者调用外部的API来执行一些指令。

semantic kernel会在合适的时候调用这些服务和插件。

如果在semantic kernel中运行任何提示或代码，内核将始终可用于检索必要的服务和插件。
...
这个向量存储连接器，主要是为了将各家的历史数据和接口描述进行向量化降维和存储，在需要的时候（比如模型生成回答）时，可以快速检索以往的上下文信息。

semantic kernel向量存储连接器提供一个抽象层，通过公共接口来呈现来自不同提供商的向量存储。 内核不会自动使用任何已注册的向量存储，但矢量搜索可以轻松地作为插件公开到内核，在这种情况下，插件可用于提示模板和聊天完成 AI 模型。

**函数和插件**

插件是函数的集合，插件具有名称。比如，插件的名称为“灯光控制组件”，可以控制物理所有灯光的开关（由于制造厂商不同，每个灯分别由不同的函数实现开关控制）。
...
这个函数可以是自己写的回调，可以是包了一层提示词的大模型，也可以是基于ElasticSearch的RAG等。
...
在任务执行期间，发生的特定事件前后自定义操作方法。比如对输入输出做一些风控过滤监测，避免涉h、摄政回答。
